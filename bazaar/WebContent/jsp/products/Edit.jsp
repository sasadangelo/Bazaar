...
<%@ page import='java.util.List, java.util.Iterator, org.opencommunity.bazaar.catalog.dto.*, org.opencommunity.bazaar.catalog.exceptions.*, org.opencommunity.bazaar.catalog.business.impl.*'%>
...
<%
    ...
    try {
        product = new ProductManagerDelegate().getProduct((Long.parseLong(request.getParameter("nID"))));
    } catch(ProductManagerException exception) {
    }
    
    try {
        allCategories = new CategoryManagerDelegate().getAllCategories();
        categoriesByProduct = new CategoryManagerDelegate().getCategoriesByProduct(Long.parseLong(request.getParameter("nID")));
    } catch(CategoryManagerException exception) {
    }
%>
...
<CENTER>
	...
	<FORM name='editProduct' action='editProduct.do' ENCTYPE='multipart/form-data' method=POST >
        <INPUT TYPE=hidden name='numId' value='<%= product.getNID() %>'>
	...
	%>
		...
		<TABLE border=0 class=Data width=100% cellspacing=1 >
			<TR>
				<TH colspan=3>Inserisci i dettagli del Prodotto</TH>
			</TR>
			<TR>
				<TD valign="top"><font color=red>*</font>Nome</TD>
				<TD valign="top"><INPUT TYPE=TEXT name='txtName' value='<%= product.getTxtName() %>'></TD>
				<TD valign="top"></TD>
			</TR>
			<TR>
				<TD valign="top">Descrizione Breve</TD>
				<TD valign="top"><INPUT TYPE=TEXT size=30 name='txtShortDescription' value='<%= product.getTxtShortDescription() %>'></TD>
				<TD valign="top"></TD>
			</TR>
			<TR>
				<TD valign="top">Descrizione</TD>
				<TD valign="top">
  				    <TEXTAREA rows=10 cols=52 name='txtDescription'>
<%=product.getTxtDescription() %>
			...
			<TR>
				...
				<TD valign="top">
				    <INPUT TYPE=file size=70 name='txtImgUrl' value=''>
				</TD>
				<TD valign="top">
			...
			<TR>
				...
				<TD valign="top"><INPUT TYPE=TEXT name='numQuantity' value='<%= product.getNumQuantity() %>'></TD>
				<TD valign="top"></TD>
			</TR>
			<TR>
				<TD valign="top"><font color=red>*</font>Prezzo (IVA inclusa)</TD>
				<TD valign="top"><INPUT TYPE=TEXT name='dblPrice' value='<%= product.getDblPrice() %>'></TD>
				<TD valign="top"></TD>
			</TR>
			<TR>
				<TD>Disponibile</TD>
				<TD>
					<SELECT name='txtActive'>
						<OPTION value='1' <%= product.isBActive() ? "selected" : "" %>>Si</OPTION>
						<OPTION value='0' <%= !product.isBActive() ? "selected" : "" %>>No</OPTION>
					</SELECT>
				</TD>
				<TD valign="top"></TD>
			</TR>
...
<!--
	function catsModified() {
		...
		var j
		vals = ""
		var sellist = document.editProduct.selectedListCategories
		...
		document.editProduct.txtAryCatIds.value = vals
	}
	...
	function addCategory() {
		...
		var val
		var bexists = false
		var list = document.editProduct.listCategories
		var sellist = document.editProduct.selectedListCategories
